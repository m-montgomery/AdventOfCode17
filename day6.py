...
def reallocate(banks):
    ...
    surplus = banks[highest]

    # redistribute count into banks
    banks[highest] = 0
    index = highest            # start at bank after bank with highest count
    while (surplus > 0):
        index = (index + 1) % len(banks)  # increment index
        banks[index] += 1                 # modify list directly for next time
        surplus -= 1                      # decrement count

    return tuple(banks)        # don't return list; will modify again later

def main():
    
    # read in starting bank block counts
    line = sys.stdin.readlines()[0].strip()
    banks = [int(num) for num in line.split()]

    cycleCount = 0              # total cycles until found repeated
    origCount = 0               # cycle number of original repeated state
    bankHistory = {}            # history of bank counts

    # reallocate until repeated
    repeated = False
    while not repeated:
        cycleCount += 1
        newBank = reallocate(banks)            # reallocate
        if newBank not in bankHistory:
            bankHistory[newBank] = cycleCount  # not repeated yet
        else:
            repeated = True                    # found repeated
            origCount = bankHistory[newBank]   # save cycle num of original

    # report number of cycles
    print("It took", cycleCount, "cycles of reallocation to repeat.")
    print("The state repeated after", cycleCount - origCount, "cycles.")
